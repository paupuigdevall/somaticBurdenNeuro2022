...
#pseudo-bulk comparisons per TP and cell-type

args=commandArgs(trailingOnly=TRUE)

timePoint <- as.character(args[1])
init <- as.numeric(args[2])
...
file <- paste0("allpools.scanpy.",timePoint,".wMetaClustUmapGraph.exprLogNormNotScaled.h5ad")
...
clustersPerGene <- sapply(rownames(seurat)[init:end], function(x){
  print(x)
  tmp_seurat <- t(as.matrix(seurat[rownames(seurat) %in% x,]@assays$RNA[]))
...
clustersPerGene <- do.call("rbind", clustersPerGene)
rownames(clustersPerGene) <- NULL


df_genes <- sapply(unique(clustersPerGene$gene), function(x){
  print(x)
  tmp <- subset(clustersPerGene, gene==x)
  
  df_gene <- sapply(unique(tmp$annot), function(y){
    print(y)
    tmp2 <- subset(tmp, annot==y)
    tmp2$zsco.cfraction <- (tmp2$cfrac-mean(tmp2$cfrac))/sd(tmp2$cfrac)
    tmp2 <- subset(tmp2, !is.na(meanExp))
    tmp2 <- subset(tmp2, nCellsLineAnnot>=10)
    tmp2$zsco.exp <- (tmp2$meanExp-mean(tmp2$meanExp))/sd(tmp2$meanExp)
    
    
    if (dim(tmp2)[1]>=10 & all(!is.nan(tmp2$zsco.exp))){
      
      data.frame(gene=x,
                 annot=y,
                 tp=timePoint,
                 nDonors=dim(tmp2)[1],
                 pval=signif(cor.test(tmp2$zsco.cfraction, tmp2$zsco.exp, method="pearson")$p.value,3),
                 corrPearson=signif(unname(cor.test(tmp2$zsco.cfraction, tmp2$zsco.exp, method="pearson")$estimate),3))
...
dirToSave <- "outputTabs/outAnalysis/"
file <- paste0(timePoint,"_clustersPerGene_",init,"_",end,".RDS")
saveRDS(df_genes, file=paste0(dirToSave,file))
