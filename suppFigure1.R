...
pathToTab <- "analysis/outputTabs/iPSC/discordant/outRDS/"
df <- paste0(pathToFolder,list.files(path=pathToFolder,pattern = ".RDS")) %>%
  map(readRDS)

names(df) <- gsub("_exclusive.+","",list.files(path=pathToFolder,pattern = ".RDS"))
df <- sapply(df, function(x) as.matrix(x), simplify=F)

dfReduced <- Reduce('+', df)
dfReduced <- as.data.frame(dfReduced)

dfReduced$log2Ratio_ptv <- log2((dfReduced$ptv_F+1)/(dfReduced$ptv_S+1))
dfReduced$log2Ratio_del <- log2((dfReduced$del_F+1)/(dfReduced$del_S+1))
dfReduced$log2Ratio_syn <- log2((dfReduced$synonymous_F+1)/(dfReduced$synonymous_S+1))

dfReduced_ddd <- dfReduced
colnames(dfReduced_ddd) <- gsub("synonymous","syn", colnames(dfReduced_ddd))

nullModel_DDD <- readRDS("analysis/outputTabs/nullDDD_1000perm.RDS")
nullModel_Cosmic <- readRDS("analysis/outputTabs/nullCosmic_1000perm.RDS")

dddAcrossEvolved <- function(dfReduced_ddd, varCategory="ptv", dataset="ddd", nullModel=nullModel_DDD){
  
  colindex <- match(paste0("log2Ratio_", varCategory), colnames(dfReduced_ddd))
  failIndex <- match(paste0(varCategory, "_F"), colnames(dfReduced_ddd))
  succIndex <- match(paste0(varCategory, "_S"), colnames(dfReduced_ddd))
  geneSetIndex <- match(dataset, colnames(geneUniverse))
  
  dfReduced_ddd_failed <- dfReduced_ddd[order(dfReduced_ddd[,colindex], decreasing=T),]
  dfReduced_ddd_succ <- dfReduced_ddd[order(dfReduced_ddd[,colindex], decreasing=F),]
  tmp1 <- dfReduced_ddd_failed[dfReduced_ddd_failed[,colindex]>1,]
  tmp2 <- dfReduced_ddd_succ[dfReduced_ddd_succ[,colindex]< -1,]
   
  allgeneSets <- cbind(as.matrix(geneUniverse[geneUniverse[,geneSetIndex],]$symbol), nullModel)
  colnames(allgeneSets)[1] <- dataset
  
  
  allPvalTab <- sapply(1:ncol(allgeneSets), function(x) {
    
    mtrixTest <- matrix(NA, 2, 2)
    rownames(mtrixTest) <- c("failed","successful")
    colnames(mtrixTest) <- c("ddd","nonddd")
    mtrixTest["failed","ddd"] <- sum(tmp1[rownames(tmp1) %in% allgeneSets[,x],failIndex])
    mtrixTest["failed","nonddd"] <- sum(tmp1[!rownames(tmp1) %in% allgeneSets[,x],failIndex])
    mtrixTest["successful","ddd"] <- sum(tmp2[rownames(tmp2) %in% allgeneSets[,x],succIndex])
    mtrixTest["successful","nonddd"] <- sum(tmp2[!rownames(tmp2) %in% allgeneSets[,x],succIndex])
    
    tt <- prop.test(mtrixTest, alternative="greater")
    data.frame(geneSetName=colnames(allgeneSets)[x],
               pval=tt$p.value)
    
  }, simplify=F)
  
  allPvalTab <- do.call("rbind", allPvalTab)
  rownames(allPvalTab) <- NULL
  
  allPvalTab <- allPvalTab[order(allPvalTab$pval, decreasing=F),]
  pval <- which(allPvalTab$geneSetName==dataset)/1000
  
  
  result <- data.frame(varType=varCategory,
                       geneSet=dataset,
                       empiricalPVal=pval)
  return(result)
...
resEnrichement <- rbind(dddAcrossEvolved(dfReduced_ddd, varCategory="ptv", dataset="ddd", nullModel=nullModel_DDD),
                        dddAcrossEvolved(dfReduced_ddd, varCategory="del", dataset="ddd", nullModel=nullModel_DDD),
                        dddAcrossEvolved(dfReduced_ddd, varCategory="syn", dataset="ddd", nullModel=nullModel_DDD),
                        dddAcrossEvolved(dfReduced_ddd, varCategory="ptv", dataset="cosmic", nullModel=nullModel_Cosmic),
                        dddAcrossEvolved(dfReduced_ddd, varCategory="del", dataset="cosmic", nullModel=nullModel_Cosmic),
                        dddAcrossEvolved(dfReduced_ddd, varCategory="syn", dataset="cosmic", nullModel=nullModel_Cosmic))


resEnrichement$signif <- pvalConverter(resEnrichement$empiricalPVal)
myPalette <- colorRampPalette(brewer.pal(9, "YlOrRd"))

shape_labeller <- rep(19,length(unique(resEnrichement$signif)))
names(shape_labeller) <- unique(resEnrichement$signif)
shape_labeller[names(shape_labeller)=="ns"] <- 1

size_labeller <- c(4,4,6)
names(size_labeller) <- c("ns","*","**")


resEnrichement$signif <- factor(resEnrichement$signif, levels=c("ns","*","**"))
resEnrichement$varType <- factor(resEnrichement$varType, levels=c("ptv","del","syn"))

my_breaks <- c(0, 0.001, 0.01, 0.05, 0.2,0.5,1)


diffMut <- ggplot(data=resEnrichement, aes(x=geneSet, y=varType, fill=empiricalPVal))+
  geom_tile()+
  geom_point(aes(size=signif, shape=signif))+
  scale_fill_gradientn(name="Empirical Pval",
                       colours=myPalette(100),
                       trans = scales::pseudo_log_trans(sigma = 0.001),
                       limits=c(0.001,1),
                       labels=my_breaks,
                       breaks=my_breaks,
                       guide = guide_colourbar(barheight = 10, nbin = 5))+
  scale_shape_manual(name="Significance", values=shape_labeller)+
  scale_size_manual(name="Significance", values=size_labeller)+
  # ylab("Variant Category")+
  # xlab("Tested gene sets")+
  xlab("")+ylab("")+
  scale_y_discrete(breaks=c("syn","del","ptv"), labels=c("Synonymous","Deleterious","LoF"))+
  scale_x_discrete(breaks=c("cosmic","ddd"), labels=c("Cosmic-Tier1","DDD"))+
  theme_bw()+
  theme(axis.title=element_text(size=14, face="bold"),
        axis.text=element_text(size=14, colour="black"),
        legend.title=element_text(size=13, face="bold"),
        legend.text=element_text(size=12))
pdf(file=paste0(pathToTabs,"figures/suppFigs/suppfig1D.pdf"), width=6, height = 4)
plot(diffMut)
dev.off()
